...
hmacData = aes_encrypt_then_hmac(key, str(clientPub.publicKey))

try:
    postURI = server + '/index.jsp'
    response = post_message(postURI, hmacData)
except:
    exit()

# decrypt the server's public key and the server nonce
packet = aes_decrypt_and_verify(key, response)
nonce = packet[0:16]
...
hmacData = aes_encrypt_then_hmac(clientPub.key, str(int(nonce)+1) + "|" + get_sysinfo())
response = post_message(postURI, hmacData)
